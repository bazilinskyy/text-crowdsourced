...
class Heroku:
    ...
    res = te.common.get_configs('kp_resolution')
    # number of stimuli
    num_stimuli = te.common.get_configs('num_stimuli')
    # number of stimuli shown for each participant
    num_stimuli_participant = te.common.get_configs('num_stimuli_participant')
    # number of repeats for each stimulus
    num_repeat = te.common.get_configs('num_repeat')
    # allowed number of stimuli with detected wrong duration
    allowed_length = te.common.get_configs('allowed_stimuli_wrong_duration')
    # allowed number of mistakes for questions with signs
    allowed_signs = te.common.get_configs('allowed_mistakes_signs')
    ...
    def read_data(self, filter_data=True):
            ...
            for row in tqdm(data_list):  # tqdm adds progress bar
                ...
                for data_cell in list_row['data']:
                    ...
                    if 'stimulus' in data_cell.keys():
                        ...
                        # instructions block
                        if (te.common.search_dict(self.prefixes, stim_no_path)
                                is not None):
                            # stimulus is found
        ...
        # save to pickle
        if self.save_p:
            te.common.save_to_p(self.file_p, df, 'heroku data')
        # save to csv
        if self.save_csv:
            # todo: check whith index=False is needed here
            df.to_csv(te.settings.output_dir + '/' + self.file_data_csv +
                      '.csv', index=False)
    ...
    def process_kp(self, filter_length=True):
        ...
        if self.save_csv:
            # save to csv
            self.mapping.to_csv(te.settings.output_dir + '/' +
    ...
    def filter_data(self, df):
        ...
        signs_answers = te.common.get_configs('signs_answers')
        for index, row in tqdm(df.iterrows(), total=df.shape[0]):
            counter_filtered = 0

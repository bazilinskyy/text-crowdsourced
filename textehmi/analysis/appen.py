...
class Appen:
    ...
    def filter_data(self, df):
        ...
        logger.info('Filter-a2. People that are under 18 years of age: {}',
                    df_2.shape[0])
        # People that took less than te.common.get_configs('allowed_min_time')
        # minutes to complete the study
        df_3 = df.loc[df['time'] < te.common.get_configs('allowed_min_time')]
        logger.info('Filter-a3. People who completed the study in under ' +
                    str(te.common.get_configs('allowed_min_time')) +
                    ' sec: {}',
                    df_3.shape[0])
        ...
        if self.save_csv:
            ...
            df_5.to_csv(te.settings.output_dir + '/' + self.file_cheaters_csv)

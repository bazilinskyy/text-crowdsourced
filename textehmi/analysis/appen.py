...
class Appen:
    ...
    columns_mapping = {'_started_at': 'start',
                       ...
                       'at_which_age_did_you_obtain_your_first_license_for_driving_a_car_or_motorcycle': 'age_license',  # noqa: E501
                       ...
                       'what_is_your_knowledge_of_english': 'prof_en',
                       'what_is_your_knowledge_of_spanish': 'prof_es',
                       'can_i_park_here': 'en1',
                       'what_colour_will_you_paint_the_childrens_bedroom': 'en2',  # noqa: E501
                       'i_cant_understand_this_email': 'en3',
                       'id_like_two_tickets_for_tomorrow_night': 'en4',
                       'shall_we_go_to_the_gym_now': 'en5'}
    ...
    def read_data(self, filter_data=True, clean_data=True):
        ...
        # load data
        if self.load_p:
            df = te.common.load_from_p(self.file_p,
                                       'appen data')
        # process data
        else:
        ...
        # save to pickle
        if self.save_p:
            te.common.save_to_p(self.file_p, df, 'appen data')
        # save to csv
        if self.save_csv:
            df.to_csv(te.settings.output_dir + '/' + self.file_csv)
    ...
    def filter_data(self, df):
        ...
        df_1 = df[df['worker_code'].duplicated(keep='first')]
        logger.info('Filter-a1. People who used the same worker_code: {}',
                    df_1.shape[0])
        # save to csv
        if self.save_csv:
            df_1 = df_1.reset_index()
            df_1.to_csv(te.settings.output_dir + '/' + self.file_cheaters_csv)
            logger.info('Filter-a1. Saved list of cheaters to csv file {}',
    ...
    def clean_data(self, df, clean_years=True):
        ...
        if clean_years:
            ...
            nans_before[1] = df['age_license'].isnull().sum()
    ...
    def mask_ips_ids(self, df, mask_ip=True, mask_id=True):
        ...
        given ID from te.common.get_configs('mask_id').
        """
        ...
        for i in range(len(df['ip'])):  # loop through ips
            ...
            if mask_id:
                ...
                if not any(d['o'] == df['worker_id'][i] for d in proc_ids):
                    ...
                    masked_id = (str(te.common.get_configs('mask_id') -
                                     df['worker_id'][i]))
                    # record IP as already replaced
    ...
    def process_countries(self):
        ...
        df_country['age_license'] = df.groupby('country').median(
            numeric_only=True).reset_index()['age_license']
        ...
        # save to csv
        if self.save_csv:
            df_country.to_csv(te.settings.output_dir + '/'
